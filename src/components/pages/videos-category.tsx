...
type Props = {
  searchParams: Omit<TVideosFilter, "category">;
  categorySlug: string;
};

export default function VideosCategory({ categorySlug, searchParams }: Props) {
  const { data, hasNextPage, fetchNextPage, refetch } =
    useInfiniteQuery<TVideosResponse>({
      queryKey: ["videos-category", categorySlug, searchParams],
      initialPageParam: 1,
      getNextPageParam: ({
        data: {
          params: { pagination },
  ...
  return (
    <>
      ...
      ></Breadcrumb>
      <div className="mb-4">
        <VideosCategoryFilter slug={categorySlug} searchParams={searchParams} />
      </div>
      {data ? (
        <InfiniteScroll
