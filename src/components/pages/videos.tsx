...
export default function Videos({ typelist, searchParams }: Props) {
  const { data, hasNextPage, fetchNextPage, refetch } =
    useInfiniteQuery<TVideosResponse>({
      queryKey: ["videos", typelist, searchParams],
      initialPageParam: 1,
      getNextPageParam: ({
        data: {
          params: { pagination },
  ...
  return (
    <>
      ...
      ></Breadcrumb>
      <div className="mb-4">
        <VideosTypelistFilter typelist={typelist} searchParams={searchParams} />
      </div>
      {data ? (
        <InfiniteScroll
