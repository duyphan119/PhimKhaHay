...
type Props = {
  searchParams: Omit<TVideosFilter, "year">;
  year: string;
};

export default function VideosYear({ year, searchParams }: Props) {
  const { data, hasNextPage, fetchNextPage, refetch } =
    useInfiniteQuery<TVideosResponse>({
      queryKey: ["videos-year", year, searchParams],
      initialPageParam: 1,
      getNextPageParam: ({
        data: {
          params: { pagination },
      ...
      queryFn: () => yearApi.fetchVideosData(year, searchParams),
    });
  ...
  return (
    <>
      ...
      ></Breadcrumb>
      <div className="mb-4">
        <VideosyearFilter slug={year} searchParams={searchParams} />
      </div>
      {data ? (
        <InfiniteScroll
